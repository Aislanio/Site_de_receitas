...
receitas.map((video) => {
  ...
  card_clone.setAttribute("data-informaçoesadicionais", video.informaçoesadicionais);
   if (receita_curtida == true) { 
    const link_curtida = document.querySelector("#receita_link")
  link_curtida.querySelector("img").src = video.img;
  link_curtida.querySelector("h2").innerHTML = video.title;
  link_curtida.querySelector("span").innerHTML = video.dificuldade;

  link_curtida.setAttribute("data-img", video.img);
  link_curtida.setAttribute("data-title", video.title);
  link_curtida.setAttribute("data-porçoes", video.porçoes);
  link_curtida.setAttribute("data-dificuldade", video.dificuldade);
  link_curtida.setAttribute("data-tempo", video.tempo);
  link_curtida.setAttribute("data-ingredientes", video.ingredientes);
  link_curtida.setAttribute("data-mododepreparo", video.mododepreparo);
  link_curtida.setAttribute("data-recheio", video.recheio);
  link_curtida.setAttribute("recheio",video.recheio)
  link_curtida.setAttribute("data-informaçoesadicionais", video.informaçoesadicionais);
    receita_curtida = false;
   }
...
function categoria(){
      localStorage.clear()
  const event_categori = event.currentTarget;
  console.log(event_categori);
  const categoria_Link = event_categori.querySelector("h2").innerText;
  localStorage.setItem("categoria_selecionada", categoria_Link);
  window.location.href = "pesquisar_receita.html";
};